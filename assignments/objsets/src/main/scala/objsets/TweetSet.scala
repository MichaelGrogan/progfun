...
class Empty extends TweetSet {
  ...
  override def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("empty set contains no most retweeted")
  /**
   * The following methods are already implemented
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    override def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      val cum = if (p(elem)) acc.incl(elem) else acc
      left.filterAcc(p, right.filterAcc(p, cum))
    }
